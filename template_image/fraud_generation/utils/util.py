...
def getsize(ffont, text):
    left, top, right, bottom = ffont.getbbox(text)
...
def get_optimal_font_scale(text, width):
    ...
    img_fraction = 1
    try:
        ffont = ImageFont.truetype(font=sel_font, size=fontsize ,encoding="unic")
    except:
        sel_font = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
        ffont = ImageFont.truetype(font=sel_font, size=fontsize ,encoding="unic")

    while (getsize(ffont, text)[0] < img_fraction*width) and (stop == False):
        # iterate until the text size is just larger than the criteria
        if getsize(ffont, text)[0] == 0:
            sel_font =  "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"

            if getsize(ffont, text)[1] == 0:
                stop = True
                break

        fontsize += 1
        ffont = ImageFont.truetype(sel_font, fontsize ,encoding="unic")

    # optionally de-increment to be sure it is less than criteria
    fontsize -= 1
    font = ImageFont.truetype(sel_font, fontsize ,encoding="unic")

    font_style = sel_font.split('/')[-1]
    return font, font_style, fontsize
...
def store1(image, path_store: str = None, save_quality: str = None):
    if path_store is None:
        path_store = os.path.join(os.getcwd(), "NeoSIDTD")


    if os.path.exists(path_store):
        path_to_save = os.path.join(path_store, image.fake_name + ".jpg")
        try:
            imageio.imwrite(path_to_save, image.fake_img)
        except:
            print(path_to_save)
            print(type(image.fake_img))
            print(image.fake_name)
