...
def inpaint_image(img: np.ndarray, coord:np.ndarray, mask: np.ndarray, text_str: str):
    ...
    color = (0, 0, 0)
    font, font_style, fontsize = ut.get_optimal_font_scale(text_str, w)
    ...
    meta_data['value'] = text_str
    meta_data['font_style'] = font_style
    meta_data['font_size'] = fontsize
    meta_data['font_color'] = color
    meta_data['bbox'] = [x0, y0, x0 + w, y0 + h]

    return fake_text_image, meta_data
