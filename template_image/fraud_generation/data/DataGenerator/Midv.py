...
class Midv():
    ...
    def get_field_info(self, info:dict, img_id1:str=None, mark:str = None, force_flag:int=1) -> Tuple[Dict, Str]:
        ...
        if (self._flag != 1 or force_flag != 1) and img_id1 is not None:
            ...
            assert type(img_id1) is str
            
            fields = info[img_id1]
            fields.pop('fraud', None)
            fields.pop('face', None)
            fields.pop('ghostimg1', None)
            fields.pop('ghostimg2', None)
            fields.pop('ghostimg', None)
            fields.pop('veteran', None)
            fields.pop('donor', None)
            fields.pop('organ_donor', None)
            fields.pop('save_quality', None)

        if not mark or mark == None:
            # (2-12) to avoid the face, the photo and the signature
            field_to_change = random.choice(list(fields.keys()))
    ...
    def Crop_and_Replace(self,img1:np.ndarray, img2:np.ndarray, info:dict, additional_info:dict, img_id1:str=None, img_id2:str=None, delta1:list=[2,2], delta2:list = [2,2], mark:str=None, force_flag:int=1) -> Tuple[Image.Image, Image.Image, Str, Str]:
        ...
        if bool(self._flag) is True or bool(force_flag) is True:
            ...
            return fake_document1, fake_document2 , swap_info_1, swap_info_2           
    ...
    def Inpaint_and_Rewrite(self,img: np.ndarray, info: dict,img_id: int=None, mark:str=None, force_flag:int=1) -> Tuple[Image.Image, Str]:
        ...
        fake_text_image, meta_data =  inpaint_image(img=img, coord=coord, mask=mask, text_str=text_str)
        swap_info['segment'] = field_to_change
        meta_data['segment'] = field_to_change
        return fake_text_image, swap_info, meta_data
